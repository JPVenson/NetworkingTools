...
using JPB.NetworkWatcher.Framework.SpeedTest.Model;
...
namespace JPB.NetworkWatcher.Framework.SpeedTest
{
	public class SpeedTestClient
	{
		...
		public double TestDownloadSpeed(Server server, int simultaniousDownloads = 2, int retryCount = 2)
		{
			...
			return TestSpeed(testData, async (client, url, stopRequested) =>
			{
		...
		public double TestUploadSpeed(Server server, int simultaniousUploads = 2, int retryCount = 2)
		{
			...
			return TestSpeed(testData, async (client, uploadData, stopRequested) =>
			{
				await client.PostAsync(server.Url, new StringContent(uploadData), stopRequested);
		...
		private static double TestSpeed<T>(IEnumerable<T> testData, Func<HttpClient, T, CancellationToken, Task<int>> doWork, int concurencyCount = 2)
		{
			var timer = new Stopwatch();
			var throttler = new SemaphoreSlim(concurencyCount);

			var cancleation = new CancellationTokenSource();
			timer.Start();
			var clients = new List<SpeedTestHttpClient>();
			var downloadTasks = testData.Select(async data =>
			{
				...
				try
				{
					var size = await doWork(client, data, cancleation.Token).ConfigureAwait(false);
					return size;
				}
				finally
				{
			...
			if (!Task.WaitAll(downloadTasks, TimeSpan.FromMinutes(1)))
			{
				cancleation.Cancel();
				foreach (var speedTestHttpClient in clients)
				{
					speedTestHttpClient.CancelPendingRequests();
				}
				return Double.NaN;
			}
			timer.Stop();
